...
ruta_ffmpeg = r"C:\Users\raulc\AppData\Local\Microsoft\WinGet\Packages\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-8.0.1-full_build\bin\ffmpeg.exe"
AudioSegment.converter = ruta_ffmpeg
AudioSegment.ffprobe = ruta_ffmpeg.replace("ffmpeg.exe", "ffprobe.exe")
...
def grabar_audio_interno(duracion_segundos=10, frecuencia_muestreo=44100, dispositivo=None):
    ...
    try:
        ...
        nombre_archivo_mp3 = f"grabacion_interna_{timestamp}.mp3"
        
        # Guardar temporalmente como WAV y convertir a MP3
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_wav:
            temp_wav_path = temp_wav.name
            wavfile.write(temp_wav_path, frecuencia_muestreo, data)
        
        try:
            # Convertir WAV a MP3 usando pydub
            audio = AudioSegment.from_wav(temp_wav_path)
            audio.export(nombre_archivo_mp3, format="mp3", bitrate="320k")
            
            ruta_completa = os.path.abspath(nombre_archivo_mp3)
            print(f"\n✅ Grabación completada!")
            print(f"📁 Archivo guardado: {ruta_completa}")
            print(f"📏 Tamaño: {os.path.getsize(nombre_archivo_mp3) / 1024:.2f} KB")
        finally:
        ...
        print("   pip install soundcard numpy scipy pydub")
        print("\n💡 Y FFmpeg instalado en tu sistema:")
        print("   Windows: scoop install ffmpeg o choco install ffmpeg")
...
def grabar_con_deteccion_pausas(umbral_silencio=0.003, duracion_pausa=1.0, frecuencia_muestreo=44100, dispositivo=None):
    ...
    try:
        ...
        buffer_audio = []
        tiempo_silencio = 0
        tamano_chunk = int(frecuencia_muestreo * 0.1)  # Chunks de 0.1 segundos
        en_silencio = False
        ...
        def guardar_archivo(data, numero):
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            nombre_archivo_mp3 = f"grabacion_{timestamp}_parte{numero:03d}.mp3"
            ...
            try:
                ...
                ruta_completa = os.path.abspath(nombre_archivo_mp3)
                tamano_kb = os.path.getsize(nombre_archivo_mp3) / 1024
                duracion_seg = len(data) / frecuencia_muestreo
                
                return {
                    'nombre': nombre_archivo_mp3,
                    'ruta': ruta_completa,
                    'tamano': tamano_kb,
            ...
            while not stop_flag.is_set():
                try:
                    ...
                    if rms < umbral_silencio:
                        ...
                        if len(buffer_audio) % 10 == 0:  # Cada segundo aproximadamente
                            duracion_actual = len(buffer_audio) * 0.1
